...
import { Download, CheckCircle, ListChecks, AlertTriangle, ArrowLeft, Zap, Box, Bot, Video, Package, Combine, FileJson, Image as ImageIcon, Sparkles, ArrowRight } from 'lucide-react';
...
import MarkdownRenderer from '@/components/MarkdownRenderer'; // Import the reusable component
...
export default function TemplateDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    if (!loading) {
      const fetchedTemplate = getTemplateBySlug(params.slug); // Use params.slug directly
  ...
  }, [params.slug, getTemplateBySlug, loading]); // Add params.slug to dependency array
  ...
  return (
    <div className="px-2 sm:px-4 py-8">
      ...
      <article className="bg-card p-6 sm:p-8 rounded-xl shadow-2xl shadow-primary/10 border border-border">
        ...
        {MainVisual}
        {/* Conditional rendering for truncated/full summary */}
        <div className="text-lg text-muted-foreground">
          <MarkdownRenderer 
            content={isSummaryExpanded ? template.summary : template.summary.split('\n').slice(0, 5).join('\n')} 
          />
          {template.summary.split('\n').length > 5 && (
            <Button 
              variant="link" 
        ...
        {/* Added button below summary */}
        {template.paymentLink ? (
          <div className="text-center mt-6">
            <a href={template.paymentLink} target="_blank" rel="noopener noreferrer">
              <Button
                size="lg"
        ...
        {template.keyFeatures && template.keyFeatures.length > 0 && (
          <>
            ...
            <section className="mb-8 bg-muted/20 border border-primary/30 rounded-lg p-6">
              <h2 className="text-2xl font-semibold mb-4 text-foreground flex items-center"><Sparkles className="mr-3 h-6 w-6 text-primary"/>Key Features</h2>
              <ul className="space-y-2">
                {template.keyFeatures.map((feature, index) => (
                  <li key={index} className="flex items-start p-3 bg-background/50 rounded-md border border-border/50 text-lg font-semibold transition-transform duration-200 hover:translate-x-2">
                    <CheckCircle className="h-5 w-5 text-green-500 mr-3 mt-1 shrink-0" /> {/* Using green checkmark for features */}
        ...
        <div className="text-center">
          {template.paymentLink ? (
           ...
           {((template.isCollection && (!parsedWorkflowFiles || parsedWorkflowFiles.length === 0) && !parseError) || (!template.isCollection && !template.templateData)) && !template.paymentLink && (
           ...
           {template.isCollection && parseError && !template.paymentLink && (
