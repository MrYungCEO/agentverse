...
export default function AdminDashboardPage() {
  ...
  const handleSaveTemplate = async (templateData: TemplateWithoutId | Template) => {
    if ('id' in templateData) { 
      await updateTemplate(templateData as Template);
      toast({ title: "Template Updated", description: `"${templateData.title}" has been updated.` });
    } else { 
      const newTemplate = await addTemplate(templateData as TemplateWithoutId);
  ...
  const filteredTemplates = templates.filter(template => {
    ...
    const title = template.title || ''; // Default to empty string if title is null or undefined
    const summary = template.summary || ''; // Default to empty string if summary is null or undefined
    ...
    if (files && files.length > 0) {
      ...
      if (invalidFiles.length > 0) {
        toast({ 
          ...
          description: `The following files were not processed as they are not JSON or HTML: ${invalidFiles.join(', ')}.`, 
          variant: "destructive",
          duration: 7000,
        });
      }
  ...
  const handleBulkUpload = async () => {
    ...
    for (const file of bulkFiles) {
      try {
        ...
        if (file.type === 'text/html') {
          ...
          const itemType: 'n8n' | 'make.com' | 'unknown' = 'unknown'; // Type is unknown for raw HTML
          const itemAdditionalContext: string | undefined = undefined; // No individual context in raw HTML

          // For HTML, the entire file content is the "workflowData"
           allItemsForBulkMode.push({
                workflowData: fileContent,
                type: itemType,
                additionalContext: itemAdditionalContext, 
                imageUrl: undefined, // No image in raw HTML
    ...
    if (bulkImportMode === 'merge') {
      if (allWorkflowFiles.length > 0) {
        const result = await bulkAddTemplates(allWorkflowFiles, 'merge', (bulkContext.trim() || overallBatchContext.trim()) || undefined);
      ...
      if (allItemsForBulkMode.length > 0) {
        const result = await bulkAddTemplates(allItemsForBulkMode, 'bulk', (bulkContext.trim() || overallBatchContext.trim()) || undefined);
        totalSuccessCount += result.successCount;
        totalErrorCount += result.errorCount;
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
             ...
             <p className="text-xs text-muted-foreground">
              ...
              <br/> - A raw HTML file.
              <br/>
              `title`, `summary`, `setupGuide`, `useCases` will be AI-generated based on file content and `additionalContext`.
            </p>
          </CardContent>
        </Card>
